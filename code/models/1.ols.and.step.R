################################################################################
#                                                                              #
# Purpose:       OLS Models / Step Selection                                   #
#                                                                              #
...
model.ols.price.preds_df <-
  data.frame(pred = predict(model.ols.price),
...
model.ols.log.from_fwd.preds_df <-
  data.frame(pred = predict(model.ols.log.from_fwd),
...
RMSE(model.ols.log.from_fwd.preds_df$pred,
     model.ols.log.from_fwd.preds_df$actual)

##MAE

MAE(model.ols.log.from_fwd.preds_df$pred,
    model.ols.log.from_fwd.preds_df$actual)
...
model.ols.log.from_bkw.preds_df <-
  data.frame(pred = predict(model.ols.log.from_bkw),
             actual = data.train$price) %>% mutate(resids = actual -
                                                     pred)
...
RMSE(model.ols.log.from_bkw.preds_df$pred,
     model.ols.log.from_bkw.preds_df$actual)

##MAE

MAE(model.ols.log.from_bkw.preds_df$pred,
    model.ols.log.from_bkw.preds_df$actual)
...
model.ols.tukey.from_fwd.preds_df <-
  data.frame(pred = predict(model.ols.tukey.from_fwd),
             actual = data.train$price) %>% mutate(resids = actual - pred)
...
model.ols.tukey.from_bkw.preds_df <-
  data.frame(pred = predict(model.ols.tukey.from_bkw),
             actual = data.train$price) %>% mutate(resids = actual - pred)
...
RMSE(
  model.ols.tukey.from_bkw.preds_df$pred,
  model.ols.tukey.from_bkw.preds_df$actual
)
