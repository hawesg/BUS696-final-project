...
model.rf.preds.train <- data.frame(
  ...
  pred = predict(rf_fit, data=wine_train) %>% round(3)
) %>% rename(actual = 1, pred = 2) %>% remove_rownames() %>%  mutate( resid = actual - pred)
model.rf.preds.test <- data.frame(
  actual=  log(wine_test$price),
  pred = predict(rf_fit, data=wine_test) %>% round(3)
) %>% rename(actual = 1, pred = 2) %>% remove_rownames() %>%  mutate( resid = actual - pred)
#library("caret")
...
ggplot(model.rf.preds.train,aes(x = pred, y = resid)) +
  ...
  labs(x="Fitted values", 
       y="Residuals", 
       title="RForest: Price in $ -- Residuals vs Fitted Plot - Train", 
       caption = "", 
       subtitle = "R2:0.612 | MSE: -0.1757")+
...
ggplot(model.rf.preds.test,aes(x = pred, y = resid)) +
  ...
  labs(x="Fitted values", 
       y="Residuals", 
       title="RForest: Price in $ -- Residuals vs Fitted Plot - Test", 
       caption = "", 
       subtitle = "")+
  geom_hline(yintercept=0, col="red", linetype="dashed")+
  theme_bw()
...
plot_predict_interaction(rf_fit, wine_train, "price", "points")
