...
.pull-right[]
.footnote[I still need to push the final github code I have been far too busy to push any changes it is 4 am now, will push code around 1pm 12/16/19 after I have a chance to clean it up for you]
...
# OVERVIEW OF BUISNESS PROPOSITION


We are trying to accurately model the price of wine utilizing several variables connected with the winemaking process.  In this dataset, the majority of the wines surveyed fall below $100, but there are notable exceptions.  This sort of model has many applications that can be directly applied to numerous businesses and ventures.  Below are some industries and vocations that could benefit from such a model. Examples of potential usecases are:
...
- The underlying assumption is that reviewers who were prolific on social media might be more generous with their assigned points then those who weren’t.
...
# knitr::kable(twitter_stats, format = "html")
```
...
- A group of colors was assigned via regular expressions ie: all varieties matching `REGEX` were classidfied as red. Validation was done of each of the matches to make sure there was no false positives.
...
- Varioius encoding issues were fixed with the text columns, NA were removed from price and columns were converted to charecter where blank implies that there was none (ie: designation).
- Wines with missing countries were removed, as were wines missing a grape varietal and taster name.
- Some coulmns were converted to charecter vectors to make them easier to work with.
...
- `by_count` superseeds the other parameters in that setting it will cause `fct_lump` based on how many observations there are with any given feature.
...
- As you can see there are distinct patterns that emerge when looking at both price and review score broken down by country.
...
- As you can see cenering and scaling increases corelation between price as well as the transforming the dependant variable with $-1 \cdot price^{-0.3}$
...
- Seems to validate Tukey's ladder provided transformation over log transformation.
...
.pull-right[
- Given the issues running the bootstrapping code from class at larger numbers and an error relating to not being able to make binary decisions above 31 levels, we investigated using `CARET` to train a bootstrapp model. I have not had time to actually work with the data but these are the results.
...
datatable(model.comparison, container = sketch, rownames = FALSE, caption = htmltools::tags$caption(
  style = 'caption-side: bottom; text-align: center;',
  '* ', htmltools::em('Test-Set predictions have not been run on bootstrap model yet')
))
```
...
#### Additionally we would like to delve deaper into the capabilities of the `caret` package specifically  hyperparameter tuning and model comparison as well as using `caretEnsemble` to run a composite model. 
